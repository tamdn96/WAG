...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh,
    float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile, int letter_box, char *img_output)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    int names_size = 0;
    ...
    while (1) {
        ...
        save_image(im, img_output);
        if (!dont_show) {
            show_image(im, img_output);
        }
...
void run_detector(int argc, char **argv)
{
    ...
    // char *img_output = find_char_arg(argc, argv, "-img-output", 0);
    ...
    // char *datacfg = argv[3];
    char *datacfg = "cfg/voc-devfest.data";
    // char *cfg = argv[4];
    char *cfg = "cfg/sonvt-yolov3-tiny.cfg";
    char *weights = "sonvt-yolov3-tiny_20000.weights";
    ...
    char *img_output = argv[4];
    // printf("%s\n", img_output);
    // char *filename = ""
    if (0 == strcmp(argv[2], "test")) test_detector(datacfg, cfg, weights, filename, thresh, hier_thresh, dont_show, ext_output, save_labels, outfile, letter_box, img_output);
